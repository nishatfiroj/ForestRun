 ...
 *(unsigned short*)0x4000208 = 0;
 ...
 if(*(volatile unsigned short*)0x4000202 & 1 << 0) {
  if (soundA.isPlaying) {
   ...
   if (soundA.vBlankCount > soundA.duration && soundA.loops) {
    playSoundA(soundA.data, soundA.length, soundA.frequency, soundA.loops);
   } else if (soundA.vBlankCount > soundA.duration) {
    soundA.isPlaying = 0;
    *(volatile unsigned short*)0x4000102 = 0;
    dma[1].cnt = 0;
   }
  ...
  if (soundB.isPlaying) {
   soundB.vBlankCount++;
   if (soundB.vBlankCount > soundB.duration && soundB.loops) {
    playSoundB(soundB.data, soundB.length, soundB.frequency, soundB.loops);
   } else if (soundB.vBlankCount > soundB.duration) {
    soundB.isPlaying = 0;
    *(volatile unsigned short*)0x4000106 = 0;
    dma[2].cnt = 0;
   }
